...
let { SalesInvoice, SalesInvoiceItem, Form, Customer, Allocation, User } = require('@src/models').tenant;

module.exports = async function getAllSalesInvoice({ currentTenantDatabase, queries }) {
  setTenantDatabase(currentTenantDatabase);
  const [queryLimit, queryPage] = [parseInt(queries.limit || 10, 10), parseInt(queries.page || 1, 10)];
  const { count: total, rows: salesInvoices } = await SalesInvoice.findAndCountAll({
    where: generateFilter(queries),
    include: [
      {
        model: Form,
        as: 'form',
        include: [
          ...
          { model: User, as: 'approvalByUser' },
        ],
      },
      { model: SalesInvoiceItem, as: 'items', include: [{ model: Allocation, as: 'allocation' }] },
...
function generateFilterFormDate(queries) {
  ...
  minDate.setDate(new Date().getDate() - 100);
...
function setTenantDatabase(currentTenantDatabase) {
  ({ SalesInvoice, SalesInvoiceItem, Form, Customer, Allocation, User } = currentTenantDatabase);
}
