...
class ProcessSendDeleteApproval {
  ...
  async call() {
    try {
      ...
      if (salesInvoiceForm.cancellationStatus !== 0) {
        return;
      }
      ...
      const { messageId, to } = await new Mailer({
        ...
        to: approver.email,
        subject: `Delete Approval Email - Sales Invoice ${salesInvoiceForm.number}`,
        html: emailBody,
      }).call();

      logger.info(
        `Sales invoice delete approval email sent, id: ${messageId}, email: ${to}, sales invoice: ${salesInvoiceForm.number}}`
      );
    } catch (error) {
      logger.error(error);
    }
...
async function generateApprovalEmailBody(
  ...
  emailBody = emailBody.replace(
    '{{approveLink}}',
    `${config.websiteUrl}/approval?tenant=${tenantName}&crud-type=delete&resource-type=SalesInvoice&action=approve&token=${emailApprovalToken}`
  );
  emailBody = emailBody.replace(
    '{{rejectLink}}',
    `${config.websiteUrl}/approval?tenant=${tenantName}&crud-type=delete&resource-type=SalesInvoice&action=reject&token=${emailApprovalToken}`
  );
