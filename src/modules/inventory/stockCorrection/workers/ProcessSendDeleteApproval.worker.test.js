...
describe('Process Send Create Approval Worker', () => {
  ...
  it('create worker', () => {
    ...
    const processSendDeleteApproval = jest.spyOn(ProcessSendDeleteApproval.prototype, 'call').mockImplementation(() => {});
    ...
    const processSendDeleteApprovalWorker = new ProcessSendDeleteApprovalWorker(params);
    processSendDeleteApprovalWorker.call();
    expect(queueProcessSpy).toHaveBeenCalled();
    expect(processSendDeleteApproval).toHaveBeenCalled();
  });
});
