  ...
  async call() {
    ...
    try {
      ...
      const { messageId, to } = await new Mailer({
        ...
        to: approver.email,
        subject: `Create Approval Email - Stock Correction ${stockCorrectionForm.number}`,
        html: emailBody,
      }).call();

      logger.info(
        `Stock correction create approval email sent, id: ${messageId}, email: ${to}, stock correction: ${stockCorrectionForm.number}}`
      );
    } catch (error) {
      logger.error(error);
    }
...
async function generateApprovalEmailBody(
  ...
  emailBody = emailBody.replaceAll(
    '{{approveLink}}',
    `${config.websiteUrl}/approval?tenant=${tenantName}&crud-type=create&resource-type=StockCorrection&action=approve&token=${emailApprovalToken}`
  );
  emailBody = emailBody.replaceAll(
    '{{rejectLink}}',
    `${config.websiteUrl}/approval?tenant=${tenantName}&crud-type=create&resource-type=StockCorrection&action=reject&token=${emailApprovalToken}`
  );
